...
class Uploader
{
    ...
    public function upload(){
        $file = $_FILES[static::ATTR_VAL_FILE];
        $filename = $_REQUEST[static::ATTR_VAL_FILENAME];

        $chunkCount = intval(@$_REQUEST[static::ATTR_VAL_CHUNK_COUNT] ?: 1);
        $chunkIndex = intval(@$_REQUEST[static::ATTR_VAL_CHUNK_INDEX] ?: 0);
    ...
    protected function saveChunk($file, $fileid, $chunkIndex){
        $savePath = $this->tempDir("{$fileid}.{$chunkIndex}.part");
        return copy($file['tmp_name'], $savePath);
    }
